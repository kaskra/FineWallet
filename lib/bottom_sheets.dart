...
class _CategoryBottomSheetState extends State<CategoryBottomSheet>
  ...
  void setScrollController() async {
    await DBProvider.db.getIndexInCategory(widget.prevSubcategory.index).then((
        idx) {
      _subcategoryScrollController = new ScrollController(
        initialScrollOffset: (_subcategory.index.toDouble() -
            (idx + midOfsubcategories)) * subcategoryCardHeight,);
      _categoryScrollController = new ScrollController(
        initialScrollOffset: (_subcategory.selectedCategory.toDouble() -
            midOfCategories) * categoryCardWidth,);
  ...
  Widget _subcategoryTile(int index, String name) {
    return Container(height: subcategoryCardHeight,
        child: Card(
            ...
            child: InkWell(child: Container(decoration: BoxDecoration(
                color: _subcategory?.index == index &&
                    _selectedCategory == _subcategory.selectedCategory ? Colors
                    .orange : Colors.white,
                border: Border.all(color: Colors.black26, width: 1,)),
                alignment: Alignment.center,
                child: Text(name)), onTap: () {
              int iconIndex = widget.isExpense == 1 ? _selectedCategory : icons
                  .length - 1;
                ...
                child: Column(children: <Widget>[
                  Container(height: categoryListHeight,
                    width: MediaQuery
                        ...
                        .width,
                    child: FutureBuilder(
                      future: widget.isExpense == 1 ? DBProvider.db
                          .getExpenseCategories() : DBProvider.db
                          .getIncomeCategory(), builder: (ctxt, snapshot) {
                      if (snapshot.hasData) {
                  ...
                  Container(height: bottomSheetHeight - categoryListHeight -
                    ...
                    child: FutureBuilder(
                      future: DBProvider.db.getSubcategoriesOfCategory(
                          widget.isExpense == 1 ? _selectedCategory + 1 : icons
                              .length), builder: (ctxt, snapshot) {
                      if (snapshot.hasData) {
                        return ListView.builder(
...
Widget buildBottomPicker(Widget picker, double bottomSheetHeight,
    ...
    Container(decoration: BoxDecoration(color: CupertinoColors.white,
        border: Border(
            top: BorderSide(color: Colors.orange, width: topBorderHeight))),
      height: bottomSheetHeight,
