...
class _CategoryBottomSheetState extends State<CategoryBottomSheet>
  ...
  void setScrollController() async {
    await DBProvider.db.getIndexInCategory(widget.prevSubcategory.index).then((idx) {
      _subcategoryScrollController = new ScrollController(initialScrollOffset: (_subcategory.index.toDouble() - (idx + midOfsubcategories)) *
          subcategoryCardHeight,);
      _categoryScrollController = new ScrollController(initialScrollOffset: (_subcategory.selectedCategory.toDouble() - midOfCategories) *
          categoryCardWidth,);
  ...
  Widget _categoryCard(IconData iconData, String label, int index) {
    return Container(
        width: categoryCardWidth, child: Material(child: InkWell(onTap: () {
      setState(() {
        _selectedCategory = index;
        _subcategoryScrollController.jumpTo(0);
      });
    },
      child: Container(decoration: BoxDecoration(color: _selectedCategory == index ? Colors.orange : Colors.grey,
  ...
  Widget _subcategoryTile(int index, String name) {
    return Container(height: subcategoryCardHeight,
        child: Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.zero),
            child: InkWell(child: Container(decoration: BoxDecoration(color: _subcategory?.index == index &&
                _selectedCategory == _subcategory.selectedCategory
                ? Colors.orange
                : Colors.white,
                border: Border.all(color: Colors.black26, width: 1,)),
                alignment: Alignment.center,
                child: Text(name)), onTap: () {
              int iconIndex = widget.isExpense == 1 ? _selectedCategory : icons.length - 1;
  ...
  Widget build(BuildContext context) {
    return Container(
        decoration: BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: Colors.orange, width: topBorderHeight))),
        height: bottomSheetHeight,
        child: GestureDetector(// Blocks taps from propagating to the modal sheet and popping.
            onTap: () {},
            child: SafeArea(top: false,
                child: Column(children: <Widget>[
                  Container(height: categoryListHeight,
                    width: MediaQuery.of(context).size.width,
                    child: FutureBuilder(future: widget.isExpense == 1
                        ? DBProvider.db.getExpenseCategories()
                        : DBProvider.db.getIncomeCategory(), builder: (ctxt, snapshot) {
                      if (snapshot.hasData) {
                        return ListView.builder(controller: _categoryScrollController,
                  ...
                  Container(height: bottomSheetHeight - categoryListHeight -
                      topBorderHeight - dividerHeight,
                    child: FutureBuilder(future: DBProvider.db.getSubcategoriesOfCategory(
                        widget.isExpense == 1 ? _selectedCategory + 1 : icons.length),
                      builder: (ctxt, snapshot) {
                        if (snapshot.hasData) {
                          return ListView.builder(controller: _subcategoryScrollController,
...
Widget buildBottomPicker(Widget picker, double bottomSheetHeight, double topBorderHeight) =>
    Container(
      decoration: BoxDecoration(color: CupertinoColors.white, border: Border(top: BorderSide(color: Colors.orange, width: topBorderHeight))),
      height: bottomSheetHeight,
      padding: const EdgeInsets.only(top: 6.0),
      child: DefaultTextStyle(
        style: const TextStyle(color: CupertinoColors.black, fontSize: 22.0,),
        child: GestureDetector(// Blocks taps from propagating to the modal sheet and popping.
          onTap: () {}, child: SafeArea(top: false, child: picker,),),),);
